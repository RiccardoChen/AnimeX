package it.project.animex_backend;
...
import it.project.animex_backend.dto.TokenMessage;
...
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT, classes = AnimexSbApplication.class)
@ActiveProfiles({"test"})
@Sql(scripts = {
        "classpath:/dbH2/00-truncate.sql",
        "classpath:/dbH2/01-insert-role.sql",
        "classpath:/dbH2/02-insert-account.sql",
        "classpath:/dbH2/03-insert-profile.sql",
        "classpath:/dbH2/04-insert-anime.sql",
        "classpath:/dbH2/05-insert-profile_anime.sql",
...
public abstract class BaseIntegrationTest {
    ...
    void setUp(){
        ...
        TokenMessage response = given()
                ...
                .then()
                .extract()
                .as(TokenMessage.class);


        this.jwtToken = response.getMessage();
    ...
    public static void verifyNoUnusedStubs(WireMockExtension... stubs) {
        for (WireMockExtension stub : stubs) {
            Set<StubMapping> usedMappings = stub.getAllServeEvents().stream()
